...
import com.entreprise.gestion.rh.repository.StatusEmployeRepository;
...
public class EmbaucheService {
    ...
    public Employe embaucherEmploye(Integer idCandidature,LocalDate debutContrat) throws Exception
    {
         ...
         * 3-reduire le nombre de postes disponibles de 1
         * 4-mettre à jour le statut du besoin si plus de postes
         * 5-creer l'entrée dans status_employe
         * 6-retourner-na ny Employe sauvegarde sao misy ilavana ny id any avy eo
         */
        ...
        // Vérifier s'il reste des postes disponibles
        if (candidature.getBesoin().getNbPosteDispo() <= 0) {
            throw new MyException("Aucun poste disponible pour cette annonce. Embauche impossible.");
        }
        
        // Réduire le nombre de postes disponibles de 1
        candidature.getBesoin().setNbPosteDispo(candidature.getBesoin().getNbPosteDispo() - 1);
        
        // Si le nombre de postes arrive à 0, mettre le statut du besoin à 0 (Fermé/Pourvu)
        if (candidature.getBesoin().getNbPosteDispo() == 0) {
            candidature.getBesoin().setStatut(0); // 0 = Statut fermé/pourvu
        }
        
        candidatureRepository.save(candidature); // Sauvegarder la modification
        ...
        // Créer l'entrée dans status_employe pour "En période d'essai"
        StatusEmploye statusEmploye = new StatusEmploye();
        statusEmploye.setLibelle("En période d'essai");
        statusEmploye.setDateEntree(LocalDateTime.now());
        statusEmploye.setEmploye(employeSauvegarde);
        statusEmployeRepository.save(statusEmploye);
