...
public class CandidatService {
    ...
    public Map<String, Object> filtrerCandidats(Map<String, Object> criteres) {
        List<Candidat> candidats = candidatRepository.findAll();
        ...
        // Convertir la liste de Candidat en Map structurée
        return convertirCandidatsEnMap(candidats, "Candidats filtrés avec succès");
    }

    public Map<String, Object> filtrerCandidatsParBesoinEtCritere(Besoin besoin, Map<String, Object> criteres) {
        ...
        List<Candidat> candidatsFiltres = candidatsPostules.stream().filter(candidat -> {
        ...
        // Convertir en Map structurée
        return convertirCandidatsEnMap(candidatsFiltres, "Candidats filtrés par besoin avec succès");
    }

    /**
     * Méthode utilitaire pour convertir une liste de Candidat en Map structurée
     */
    private Map<String, Object> convertirCandidatsEnMap(List<Candidat> candidats, String message) {
        Map<String, Object> result = new HashMap<>();
    ...
    /**
     * Nouvelle méthode pour lister tous les candidats avec pagination
     */
    @Transactional(readOnly = true)
    public Map<String, Object> listerCandidatsAvecPagination(int page, int size) {
        // Implémentez votre logique de pagination ici
        // Pour l'instant, on retourne tous les candidats
        List<Candidat> candidats = candidatRepository.findAll();
        return convertirCandidatsEnMap(candidats, "Candidats récupérés avec succès");
    }

    /**
     * Nouvelle méthode pour rechercher des candidats par nom ou prénom
     */
    @Transactional(readOnly = true)
    public Map<String, Object> rechercherCandidatsParNom(String recherche) {
        List<Candidat> candidats = candidatRepository.findAll().stream()
            .filter(c -> c.getPersonne().getNom().toLowerCase().contains(recherche.toLowerCase()) ||
                        c.getPersonne().getPrenom().toLowerCase().contains(recherche.toLowerCase()))
            .collect(Collectors.toList());
        
        return convertirCandidatsEnMap(candidats, "Candidats recherchés avec succès");
    }
