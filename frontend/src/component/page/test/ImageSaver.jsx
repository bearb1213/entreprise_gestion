...
const ImageSaver = () => {
  ...
  const saveImage = async () => {
    ...
    try {
        ...
        // CORRECTION : Utiliser 'image' au lieu de 'file'
        dataSend.append('image', selectedImage);
        
        console.log("Envoi de l'image:", selectedImage.name);
        
        const response = await fetch("http://localhost:8080/api/candidats/sary", {
            method: "POST",
            body: dataSend,
            credentials: "include"
        });
        ...
        if (response.ok) {
            ...
            throw new Error(errorText || "Erreur lors de l'envoi de la candidature");
        }
    } catch (error) {
