...
const LogEmploye = () => {
    ...
    // Fonction pour extraire le rôle des données utilisateur
    const getUserRole = (userData) => {
        if (!userData || !userData.roles) return null;
        
        console.log("Données utilisateur pour extraction du rôle:", userData);
        
        // Si les rôles sont dans un tableau d'authorities
        if (Array.isArray(userData.roles)) {
            const roleAuthority = userData.roles.find(auth => 
            ...
            // Si les rôles sont directement dans un tableau de strings
            const roleString = userData.roles.find(role => 
    ...
    const getRedirectPath = (userRole) => {
        console.log("Rôle détecté pour redirection:", userRole);
        ...
        switch (userRole) {
            ...
            case 'RH':
                return '/dashboard-rh';
            case 'DEPARTEMENT':
                return '/dashboard-departement';
            case 'UNITE':
                return '/dashboard-unite';
            case 'CANDIDAT':
                return '/dashboard';
            default:
                return '/';
    ...
    // Fonction pour récupérer les infos utilisateur après login
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Étape 1: Login
            if (typeLog === "EMP") {
                response = await fetch("http://localhost:8080/api/utilisateur/login", {
                    method: 'POST',
                    headers: {
            ...
            if (response.ok) {
                ...
                // Étape 2: Récupérer les infos utilisateur avec /me
                ...
                setErrors({ 
                    general: typeLog === "EMP" 
                        ...
                        : 'Identifiant incorrect ou non autorisé' 
                });
            }
